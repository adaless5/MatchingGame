...
void ImGuiManager::OnEvent(Event* pEvent)
{
    ...
    if( pEvent->GetType() == "InputEvent" )
    {
        ...
        if( pInputEvent->GetDevice() == InputDevice::Keyboard )
        {
            if( pInputEvent->GetState() == InputState::Pressed )
            {
                io.KeysDown[pInputEvent->GetKeyCode()] = true;
            }

            if( pInputEvent->GetState() == InputState::Released )
            {
                io.KeysDown[pInputEvent->GetKeyCode()] = false;
            }
        }

        if( pInputEvent->GetDevice() == InputDevice::Mouse )
        {
            io.MousePos.x = (float)pInputEvent->GetPosition().x;
            io.MousePos.y = (float)pInputEvent->GetPosition().y;

            if( pInputEvent->GetState() == InputState::Pressed )
            {
                io.MouseDown[pInputEvent->GetMouseButtonID()] = true;
            }
            if( pInputEvent->GetState() == InputState::Released )
            {
                io.MouseDown[pInputEvent->GetMouseButtonID()] = false;
            }
        }
    }
...
void ImGuiManager::StartFrame(float width, float height, float deltaTime)
{
    ImGui::NewFrame();
